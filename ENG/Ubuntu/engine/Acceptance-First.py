...
─────────────────────────────────────────────────────────────────────────────
ATDD — reference pipeline: per-user-story batches + overlay (impact review, LLM verifier, breakage triage).

ATDD base (ex-ATDD, removed: this script is now the reference), enriched with the
...
Reminder of the ATDD base — difference with the Test-First.py pipeline:
...
def build_coder_prompt(phase: dict, blackboard: dict, user_need: str, skills_context: str,
    ...
    if nature == TEST_NATURE:
        ...
        prod_edit_policy = ("In an implementation phase, you create and modify NO test file "
                            ...
                            "or an earlier batch may have left a bug to fix).") + planned_test_changes_policy(phase)
...
def write_fail_report(title: str, reason: str, blackboard: dict = None, details: str = "",
    ...
    mm_audit.end("failed")
    try:
        lines = ["# Failure report — Acceptance-First", "", f"## {title}", "", "### Cause", reason.strip(), ""]
...
def main():
    ...
    if os.path.exists(IMPACT_APPROVED_SENTINEL):
        os.remove(IMPACT_APPROVED_SENTINEL)
    print("\n🏁 [CONGRATULATIONS] The Acceptance-First factory closed all its batches (acceptance tests → implementation → refactor) in a single run!")

...
─────────────────────────────────────────────────────────────────────────────
TDD — reference pipeline: red → green → refactor cycles + overlay (impact review, LLM verifier, breakage triage).

TDD base (ex-TDD, removed: this script is now the reference), enriched with the
Yolo overlay — the arbitration logic of Guided-Fix.py moved INTO the run:
  ...
  - Green path ('tdd-green' phases only): after the mechanical verdict, an independent
    LLM Verifier (fresh context) confronts the code actually produced with the phase's
    checklist in the blackboard — a green suite proves "nothing is broken", not "the
    phase delivered everything". It never stamps DONE: a rejection consumes one attempt.
  - Red path ('tdd-green' phases only): a Triage Agent confronts the failing tests with
    'impact.md' — ENDORSED breakage → the test is deleted by the ORCHESTRATOR (never by
    an agent, accounting adjusted) and the flow continues; UNPLANNED breakage → a
    Repairer Agent fixes the side effect (failing tests FROZEN, the phase's behavior
    still required); a TRUE conflict (the two behaviors exclude each other) →
    'impact-phase-<id>.md' is arbitrated by the human at a mid-run gate (accepted →
    mechanical deletion; rejected → the historical behavior prevails, the fix is
    recorded). The REJECTED + failReport.md net after MAX_ATTEMPTS is unchanged — it
    should just become rare.
  - Scope: the 'tdd-red' phases (INVERTED verdict: a red there is a success) are
    UNCHANGED — impact triage and the verifier make no sense there.

Reminder of the TDD base — difference with the "universal verdict" pipeline Coding.py:
...
RUNNER = resolve_runner(os.getcwd(), role="test-first")
...
def build_coder_prompt(phase: dict, blackboard: dict, user_need: str,
    ...
    if nature == RED_NATURE:
        ...
        prod_edit_policy = ("In a green phase, you create and modify NO test file "
                            ...
                            "earlier cycle to fix).") + planned_test_changes_policy(phase)
...
def write_fail_report(title: str, reason: str, blackboard: dict = None, details: str = "",
    ...
    mm_audit.end("failed")
    try:
        lines = ["# Failure report — Test-First", "", f"## {title}", "", "### Cause", reason.strip(), ""]
...
def run_production_phases(blackboard: dict, user_need: str, need_is_spec: bool = False):
    ...
    for phase in blackboard["phases"]:
        ...
        while not success and attempts < MAX_ATTEMPTS:
            ...
            # ── YOLO · RED PATH (green phases only) ── : before handing a red back to the
            # coder, the impact triage tries to resolve it — breakage endorsed by the human
            # → mechanical deletion of the test and carry on; unplanned side effect →
            # repairer (tests frozen); a true conflict → human arbitration
            # (impact-phase-<id>.md). If it turns the suite green, the green carries on below
            # (LLM verifier included); otherwise the freshest red output becomes the coder's
            # feedback again. The red phases (inverted verdict) NEVER come through here.
            if not is_ok:
            ...
            if is_ok:
                ...
                # ── YOLO · PHASE LLM VERIFIER (green path, green phases only) ── : a green
                # suite proves "nothing is broken", not "the phase delivered everything".
                # An independent fresh-context agent confronts the produced code with the
                # phase's checklist in the blackboard. It never stamps DONE: a rejection
                # (or the absence of a verdict) consumes the attempt and hands back to the
                # coder — placed BEFORE brick B so that no mutation run is paid for on an
                # attempt that is going to be rejected.
...
def main():
    ...
    mm_audit.start(os.getcwd(), "test-first", RUNNER.name,
                   model=RUNNER.configured_model())
    check_need_file()
    ...
    if os.path.exists(IMPACT_APPROVED_SENTINEL):
        os.remove(IMPACT_APPROVED_SENTINEL)
    print("\n🏁 [CONGRATULATIONS] The Test-First factory closed all its cycles (red → green → refactor) in a single run!")

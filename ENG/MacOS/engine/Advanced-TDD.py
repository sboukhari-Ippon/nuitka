...
─────────────────────────────────────────────────────────────────────────────
"YOLO-TDD" VARIANT (TDD + Yolo overlay: impact review, LLM verifier, breakage triage).

Derives from Safe-TDD.py (full copy, the base is NOT modified), enriched with the
...
Reminder of the TDD base (unchanged) — difference with the "universal verdict" variant Safe-Coding.py:
...
def write_fail_report(title: str, reason: str, blackboard: dict = None, details: str = ""):
    ...
    mm_audit.end("failed")
    try:
        lines = ["# Failure report — Advanced-TDD", "", f"## {title}", "", "### Cause", reason.strip(), ""]
...
def main():
    ...
    if os.path.exists(IMPACT_APPROVED_SENTINEL):
        os.remove(IMPACT_APPROVED_SENTINEL)
    print("\n🏁 [CONGRATULATIONS] The Advanced-TDD factory closed all its cycles (red → green → refactor) in a single run!")

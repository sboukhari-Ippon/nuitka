...
# -*- coding: utf-8 -*-
"""
AI Orchestrator - ACCESSIBILITY AUDIT factory with an agent harness + tmux (RGAA 4.1.2)
─────────────────────────────────────────────────────────────────────────────────
"A11Y AUDIT" VARIANT: it writes NO code — it evaluates an EXISTING web interface
against the 106 criteria of RGAA 4.1.2 (13 topics) and delivers a consolidated report
'accessibility_audit_report.md' (C/NC/NA/AVM statuses per criterion, conformance rate
as a range, localized findings) plus a pre-filled accessibility declaration skeleton
('declaration_accessibilite.md').
...
Pipeline:
  ...
  - Step S4: aggregation and 100% Python report (consolidation NC > AVM > C > NA,
    ...
    accessibility declaration skeleton.
...
ALWAYS replayed. To redo a full audit: delete 'audit_a11y/' (and 'a11y_map.yaml' to replay
...
NEED_FILE             = "need.md"
SPEC_FILE             = "spec.md"
A11Y_DIR              = "audit_a11y"                       # intermediate verdicts (one file per pass)
A11Y_MAP_FILE         = "a11y_map.yaml"                    # interface map (the equivalent of the blackboard)
A11Y_REPORT_FILE      = "accessibility_audit_report.md"    # consolidated final deliverable, at the ROOT
A11Y_DECLARATION_FILE = "declaration_accessibilite.md"     # pre-filled declaration skeleton, at the ROOT
...
A11Y_MARKER           = "<!-- généré par Audit-A11Y-RGAA -->"
...
MAX_FILES_PER_PASS = 25              # L7: beyond this, the compartment is SPLIT into slices (the mapper grid's bound)
...
_A11Y_BASENAMES = {A11Y_REPORT_FILE, A11Y_DECLARATION_FILE, A11Y_MAP_FILE,
...
def build_pass_list(buckets: list, packs: list, triggers: dict) -> list:
    ...
    for bucket in buckets:
        slices = [bucket["files"][i:i + MAX_FILES_PER_PASS]
                  for i in range(0, len(bucket["files"]), MAX_FILES_PER_PASS)] or [list(bucket["files"])]
...
def summarize_by_directory(files: list) -> str:
    """Per-directory summary of the files NOT listed in the cartography prompt
    (assumed, displayed degradation: they end up in the "Miscellaneous" zone via coverage)."""
    counts = {}
    for f in files:
        d = os.path.dirname(f) or "."
        counts[d] = counts.get(d, 0) + 1
    return "\n".join(f"- {d}/: {n} unlisted file(s)" for d, n in sorted(counts.items()))


def build_carto_scope_block(scope_files: list) -> str:
    """The "files to assign" block of the cartographer prompt, capped at
    MAX_SCOPE_FILES_IN_CARTO (the overflow is summarized per directory)."""
    listed = scope_files[:MAX_SCOPE_FILES_IN_CARTO]
    block = "\n".join(f"- {f}" for f in listed) or "(none)"
    overflow = scope_files[len(listed):]
    if overflow:
        block += (f"\n(⚠️ Scope truncated to {MAX_SCOPE_FILES_IN_CARTO} files: "
                  f"{len(overflow)} unlisted file(s), summarized per directory "
                  f"below. Do NOT assign them: the orchestrator will add them "
                  f"mechanically to the \"Miscellaneous\" zone.)\n"
...
def print_a11y_map_recap(a11y_map: dict, soft: list, packs: list, triggers: dict):
    ...
    print(f"🗺️  INTERFACE MAP — {a11y_map.get('project', '(unnamed)')}: "
          f"{len(buckets)} compartment(s), {len(passes)} audit pass(es)")
...
def confirm_a11y_map(a11y_map: dict, soft: list, packs: list, triggers: dict):
    ...
    confirm = input("\n▶️  Validate this map and launch the accessibility audit? (y/n): ")
...
def warn_orphan_findings(passes: list):
    """Files in 'audit_a11y/' matching no pass of the matrix (map re-edited by
...
def aggregate(passes: list, packs: list) -> dict:
    ...
    Returns {"criteria": {criterion: {...}}, "topics": [...], "totals": {...},
             ...
             "unreadable": [...]} — the sole material for the report and the declaration.
    """
    criteria = {}
    for pack in packs:
        for crit in pack["criteres"]:
...
# ─── STEP S4 (DELIVERABLES): REPORT & DECLARATION (PYTHON, ATOMIC WRITE) ─────
...
def assemble_report(stats: dict, a11y_map: dict, passes: list, packs: list,
    ...
    parts = [f"# Accessibility audit (RGAA 4.1.2) — {project}", "", A11Y_MARKER, "",
             f"*Generated on {time.strftime('%Y-%m-%d')} by `Audit-A11Y-RGAA.py` — "
             f"automated STATIC pre-audit: {len(passes)} audit pass(es) "
             f"(pack × compartment), {len(scope_files)} UI file(s), "
...
def write_declaration(stats: dict, a11y_map: dict) -> None:
    """Accessibility declaration skeleton pre-filled with the pre-audit figures.
    Administrative fields left as [TO COMPLETE]; warning at the top: the legal
    declaration requires a full conformity audit."""
    totals = stats["totals"]
    project = str(a11y_map.get("project") or os.path.basename(os.getcwd()))
    if totals["NC"] == 0 and totals["AVM"] == 0:
        etat = "fully compliant"
    elif stats["rate_floor"] >= 50.0:
        etat = "partially compliant"
    else:
        etat = ("non-compliant (based on the floor rate; a full audit may "
                "reclassify this state)")
    ncs = sorted((c for c, e in stats["criteria"].items() if e["statut"] == "NC"),
                 key=criteria_sort_key)
    nc_lines = [f"- Criterion {crit} ({stats['criteria'][crit]['pack']['nom']}): "
                f"{stats['criteria'][crit]['constats'][0]['titre'] if stats['criteria'][crit]['constats'] else '(see report)'}"
                for crit in ncs] or ["- (no non-conformity demonstrated by the static pre-audit)"]

    parts = [f"# Déclaration d'accessibilité — {project} (SKELETON TO COMPLETE)", "",
             A11Y_MARKER, "",
             "> ⚠️ **This document is a SKELETON pre-filled by an automated static "
             "pre-audit** (`Audit-A11Y-RGAA.py`). It is NOT publishable as-is: "
             "the regulation requires a declaration based on a full conformity "
             "audit (manual verifications and tests with assistive technologies "
             "included) and accurate administrative information. Use it as a working "
             "base.", "",
             "[TO COMPLETE: entity] undertakes to make its service accessible, in accordance "
             "with l'article 47 de la loi n° 2005-102 du 11 février 2005.", "",
             f"This déclaration d'accessibilité applies to **{project}** "
             "[TO COMPLETE: service URL].", "",
             "## Compliance status", "",
             f"**{project}** is **{etat}** with the référentiel général d'amélioration de "
             f"l'accessibilité (RGAA), version 4.1.2. [TO CONFIRM with a full audit.]", "",
             "## Test results", "",
             f"The static pre-audit performed on {time.strftime('%Y-%m-%d')} reveals a "
             f"demonstrable compliance of **{stats['rate_central']}%** of the applicable "
             f"RGAA criteria (range from {stats['rate_floor']}% to {stats['rate_ceiling']}%: "
             f"{totals['AVM']} criterion(s) remain to be verified manually).", "",
             "## Non-accessible content", "",
             "The following non-conformities are demonstrated by the pre-audit (detail, "
             f"locations and fixes in '{A11Y_REPORT_FILE}'):", ""]
    parts += nc_lines
    parts += ["",
              "## Establishment of this declaration", "",
              "- Date: [TO COMPLETE after the full conformity audit].",
              "- Technologies used: [TO COMPLETE].",
              "- Method: automated static pre-audit (to be completed with manual "
              "tests: keyboard, screen readers, 200% zoom, contrasts on rendering).", "",
              "## Feedback and contact", "",
              "[TO COMPLETE: accessible means of contact to report an accessibility "
              "defect and to request content in an alternative form.]", "",
              "## Means of redress", "",
              "[TO COMPLETE: reminder of the possibility to refer the matter to the Défenseur des droits in "
              "case of no response.]", ""]

    atomic_write(A11Y_DECLARATION_FILE, "\n".join(parts))
    print(f"✅ Pre-filled declaration skeleton: '{A11Y_DECLARATION_FILE}' (state: {etat}).")
...
def main():
    ...
    mm_audit.start(os.getcwd(), "audit-a11y", RUNNER.name,
                   model=RUNNER.configured_model())
    ...
    print(f"\n{'='*50}")
    print(f"♿ ACCESSIBILITY AUDIT (RGAA 4.1.2) — Discovered scope:")
    ...
    if existing_map:
        ...
        print(f"   Flow: 1 cartography (skipped if '{A11Y_MAP_FILE}' valid) + N audit "
              ...
              f"→ '{A11Y_REPORT_FILE}' + '{A11Y_DECLARATION_FILE}' (root).")
    for deliverable in (A11Y_REPORT_FILE, A11Y_DECLARATION_FILE):
    ...
    confirm = input("\n▶️  Run the accessibility audit on this scope? (y/n): ")
    ...
    # Step S4 (deliverables): report + declaration skeleton.
    assemble_report(stats, a11y_map, passes, packs, contrasts, scope_files, failed_passes,
                    trigger_hits, sonde_hits)
    write_declaration(stats, a11y_map)
    ...
    if failed_passes:
        ...
        write_fail_report(f"Partial audit: {len(failed_passes)} pass(es) not completed",
    ...
    totals = stats["totals"]
    print(f"""
🏁 [CONGRATULATIONS] Accessibility audit complete!
   ...
   📄 Declaration skeleton: '{A11Y_DECLARATION_FILE}' (to complete, not publishable as-is).

...
─────────────────────────────────────────────────────────────────────────────
"ATDD" VARIANT (Acceptance Test-Driven Development driven by the plan: batches per user story).

Difference with the "TDD" variant Safe-TDD.py (from which this script derives):
...
def write_fail_report(title: str, reason: str, blackboard: dict = None, details: str = ""):
    ...
    mm_audit.end("failed")
    try:
        lines = ["# Failure report — Safe-ATDD", "", f"## {title}", "", "### Cause", reason.strip(), ""]
...
def main():
    ...
    if os.path.exists(SPEC_APPROVED_SENTINEL):
        os.remove(SPEC_APPROVED_SENTINEL)
    print("\n🏁 [CONGRATULATIONS] The ATDD factory closed all its batches (acceptance tests → implementation → refactor) in a single run!")

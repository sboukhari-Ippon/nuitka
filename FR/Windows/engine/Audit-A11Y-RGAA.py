...
# -*- coding: utf-8 -*-
"""
Orchestrateur IA - Usine à AUDIT D'ACCESSIBILITÉ avec un harness d'agent + tmux (RGAA 4.1.2)
─────────────────────────────────────────────────────────────────────────────────
VARIANTE « AUDIT A11Y » : elle n'écrit AUCUN code — elle évalue une interface web
EXISTANTE contre les 106 critères du RGAA 4.1.2 (13 thématiques) et livre un rapport
consolidé 'accessibility_audit_report.md' (statuts C/NC/NA/AVM par critère, taux de
conformité en fourchette, constats localisés) plus un squelette de déclaration
d'accessibilité pré-rempli ('declaration_accessibilite.md').
...
Pipeline :
  ...
  - Étape É4 : agrégation et rapport 100 % Python (consolidation NC > AVM > C > NA,
    ...
    squelette de déclaration d'accessibilité.
...
complet : supprimer 'audit_a11y/' (et 'a11y_map.yaml' pour rejouer la carte) et relancer.
...
NEED_FILE             = "need.md"
SPEC_FILE             = "spec.md"
A11Y_DIR              = "audit_a11y"                       # verdicts intermédiaires (un fichier par passe)
A11Y_MAP_FILE         = "a11y_map.yaml"                    # carte d'interface (l'équivalent du blackboard)
A11Y_REPORT_FILE      = "accessibility_audit_report.md"    # livrable final consolidé, à la RACINE
A11Y_DECLARATION_FILE = "declaration_accessibilite.md"     # squelette de déclaration pré-rempli, à la RACINE
...
MAX_FILES_PER_PASS = 25              # L7 : au-delà, le compartiment est SCINDÉ en tranches (borne de la grille du cartographe)
...
def build_carto_scope_block(scope_files: list) -> str:
    ...
    MAX_SCOPE_FILES_IN_CARTO (le surplus est résumé par répertoire)."""
...
def print_a11y_map_recap(a11y_map: dict, soft: list, packs: list, triggers: dict):
    ...
    print(f"🗺️  CARTE D'INTERFACE — {a11y_map.get('project', '(sans nom)')} : "
          f"{len(buckets)} compartiment(s), {len(passes)} passe(s) d'audit")
...
def confirm_a11y_map(a11y_map: dict, soft: list, packs: list, triggers: dict):
    ...
    confirm = input("\n▶️  Valider cette carte et lancer l'audit d'accessibilité ? (y/n) : ")
...
def warn_orphan_findings(passes: list):
    """Fichiers de 'audit_a11y/' ne correspondant à aucune passe de la matrice (carte
...
def aggregate(passes: list, packs: list) -> dict:
    ...
    Renvoie {"criteria": {critère: {...}}, "topics": [...], "totals": {...},
             ...
             "unreadable": [...]} — la matière unique du rapport et de la déclaration.
    """
    criteria = {}
    for pack in packs:
        for crit in pack["criteres"]:
...
# ─── ÉTAPE É4 (LIVRABLES) : RAPPORT & DÉCLARATION (PYTHON, ÉCRITURE ATOMIQUE) ─
...
def assemble_report(stats: dict, a11y_map: dict, passes: list, packs: list,
    ...
    parts = [f"# Audit d'accessibilité (RGAA 4.1.2) — {project}", "", A11Y_MARKER, "",
             f"*Généré le {time.strftime('%Y-%m-%d')} par `Audit-A11Y-RGAA.py` — "
             f"pré-audit STATIQUE automatisé : {len(passes)} passe(s) d'audit "
             f"(pack × compartiment), {len(scope_files)} fichier(s) UI, "
...
def write_declaration(stats: dict, a11y_map: dict) -> None:
    """Squelette de déclaration d'accessibilité pré-rempli avec les chiffres du pré-audit.
    Champs administratifs laissés [À COMPLÉTER] ; avertissement en tête : la déclaration
    légale exige un audit de conformité complet."""
    totals = stats["totals"]
    project = str(a11y_map.get("project") or os.path.basename(os.getcwd()))
    if totals["NC"] == 0 and totals["AVM"] == 0:
        etat = "totalement conforme"
    elif stats["rate_floor"] >= 50.0:
        etat = "partiellement conforme"
    else:
        etat = ("non conforme (sur la base du taux plancher ; un audit complet peut "
                "requalifier cet état)")
    ncs = sorted((c for c, e in stats["criteria"].items() if e["statut"] == "NC"),
                 key=criteria_sort_key)
    nc_lines = [f"- Critère {crit} ({stats['criteria'][crit]['pack']['nom']}) : "
                f"{stats['criteria'][crit]['constats'][0]['titre'] if stats['criteria'][crit]['constats'] else '(voir rapport)'}"
                for crit in ncs] or ["- (aucune non-conformité démontrée par le pré-audit statique)"]

    parts = [f"# Déclaration d'accessibilité — {project} (SQUELETTE À COMPLÉTER)", "",
             A11Y_MARKER, "",
             "> ⚠️ **Ce document est un SQUELETTE pré-rempli par un pré-audit statique "
             "automatisé** (`Audit-A11Y-RGAA.py`). Il n'est PAS publiable en l'état : "
             "la réglementation exige une déclaration fondée sur un audit de conformité "
             "complet (vérifications manuelles et tests avec technologies d'assistance "
             "compris) et des informations administratives exactes. Utilise-le comme base "
             "de travail.", "",
             "[À COMPLÉTER : entité] s'engage à rendre son service accessible, conformément "
             "à l'article 47 de la loi n° 2005-102 du 11 février 2005.", "",
             f"Cette déclaration d'accessibilité s'applique à **{project}** "
             "[À COMPLÉTER : URL du service].", "",
             "## État de conformité", "",
             f"**{project}** est **{etat}** avec le référentiel général d'amélioration de "
             f"l'accessibilité (RGAA), version 4.1.2. [À CONFIRMER par un audit complet.]", "",
             "## Résultats des tests", "",
             f"Le pré-audit statique réalisé le {time.strftime('%Y-%m-%d')} révèle une "
             f"conformité démontrable de **{stats['rate_central']} %** des critères RGAA "
             f"applicables (fourchette de {stats['rate_floor']} % à {stats['rate_ceiling']} % : "
             f"{totals['AVM']} critère(s) restent à vérifier manuellement).", "",
             "## Contenus non accessibles", "",
             "Les non-conformités suivantes sont démontrées par le pré-audit (détail, "
             f"localisations et corrections dans '{A11Y_REPORT_FILE}') :", ""]
    parts += nc_lines
    parts += ["",
              "## Établissement de cette déclaration", "",
              "- Date : [À COMPLÉTER après l'audit de conformité complet].",
              "- Technologies utilisées : [À COMPLÉTER].",
              "- Méthode : pré-audit statique automatisé (à compléter par des tests "
              "manuels : clavier, lecteurs d'écran, zoom 200 %, contrastes au rendu).", "",
              "## Retour d'information et contact", "",
              "[À COMPLÉTER : moyen de contact accessible permettant de signaler un défaut "
              "d'accessibilité et de demander les contenus sous une autre forme.]", "",
              "## Voies de recours", "",
              "[À COMPLÉTER : rappel de la possibilité de saisir le Défenseur des droits en "
              "cas de non-réponse.]", ""]

    atomic_write(A11Y_DECLARATION_FILE, "\n".join(parts))
    print(f"✅ Squelette de déclaration pré-rempli : '{A11Y_DECLARATION_FILE}' (état : {etat}).")
...
def main():
    ...
    print(f"\n{'='*50}")
    print(f"♿ AUDIT D'ACCESSIBILITÉ (RGAA 4.1.2) — Périmètre découvert :")
    ...
    if existing_map:
        ...
        print(f"   Déroulé : 1 cartographie (sautée si '{A11Y_MAP_FILE}' valide) + N passes "
              ...
              f"→ '{A11Y_REPORT_FILE}' + '{A11Y_DECLARATION_FILE}' (racine).")
    ...
    confirm = input("\n▶️  Lancer l'audit d'accessibilité sur ce périmètre ? (y/n) : ")
    ...
    # Étape É4 (livrables) : rapport + squelette de déclaration.
    ...
    if failed_passes:
        ...
        write_fail_report(f"Audit partiel : {len(failed_passes)} passe(s) non abouties",
    ...
    totals = stats["totals"]
    print(f"""
🏁 [CONGRATULATIONS] Audit d'accessibilité terminé !
   ...
   📄 Squelette de déclaration : '{A11Y_DECLARATION_FILE}' (à compléter, non publiable en l'état).

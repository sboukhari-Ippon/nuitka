...
─────────────────────────────────────────────────────────────────────────────
VARIANTE « ATDD » (Acceptance Test-Driven Development piloté par le plan : lots par user story).

Différence avec la variante « TDD » Safe-TDD.py (dont ce script dérive) :
...
import mm_core
from mm_core import (
    apply_blackboard_defaults, build_refacto_fix_prompt, build_skills_dictionary, collect_spec_us_ids,
    commit_phase, done_sentinel, ensure_phase_repo, fail_pipeline,
    files_changed_since_phase_start, git_head_sha, inject_skills_dictionary, load_blackboard,
    load_skills, lot_closing_ids, mutation_tool_available, no_declared_file_touched,
    parse_test_count, read_touched_files, resolve_build_cmd, resolve_mutation_cmd,
    resolve_verify_cmd, run_mutation, run_verify, run_verify_resilient,
    save_blackboard, signal_handler, test_count_regression, test_phase_damage,
    truncate_output, validate_all_skills, wait_for_file_creation,
)
...
RUNNER = resolve_runner(os.getcwd(), role="atdd")
...
def build_coder_prompt(phase: dict, blackboard: dict, user_need: str, skills_context: str,
    ...
    if nature == TEST_NATURE:
        ...
        prod_edit_policy = ("En phase d'implémentation, tu ne crées et ne modifies AUCUN fichier "
                            ...
                            "ou un lot antérieur peut avoir laissé un bug à corriger).")
...
def write_fail_report(title: str, reason: str, blackboard: dict = None, details: str = ""):
    ...
    mm_audit.end("failed")
    try:
        lines = ["# Rapport d'échec — Safe-ATDD", "", f"## {title}", "", "### Cause", reason.strip(), ""]
...
def main():
    ...
    mm_audit.start(os.getcwd(), "atdd", RUNNER.name,
                   model=RUNNER.configured_model())
    check_need_file()
    ...
    if os.path.exists(SPEC_APPROVED_SENTINEL):
        os.remove(SPEC_APPROVED_SENTINEL)
    print("\n🏁 [CONGRATULATIONS] L'usine ATDD a refermé tous ses lots (tests d'acceptance → implémentation → refactor) en un seul run !")

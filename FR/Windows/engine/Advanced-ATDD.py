...
─────────────────────────────────────────────────────────────────────────────
VARIANTE « YOLO-ATDD » (ATDD + surcouche Yolo : revue d'impact, vérificateur LLM, triage des cassures).

Dérive de Safe-ATDD.py (copie intégrale, la base n'est PAS modifiée), enrichie de
...
Rappel de la base ATDD (inchangée) — différence avec la variante « TDD » Safe-TDD.py :
...
def write_fail_report(title: str, reason: str, blackboard: dict = None, details: str = ""):
    ...
    mm_audit.end("failed")
    try:
        lines = ["# Rapport d'échec — Advanced-ATDD", "", f"## {title}", "", "### Cause", reason.strip(), ""]
...
def main():
    ...
    if os.path.exists(IMPACT_APPROVED_SENTINEL):
        os.remove(IMPACT_APPROVED_SENTINEL)
    print("\n🏁 [CONGRATULATIONS] L'usine Advanced-ATDD a refermé tous ses lots (tests d'acceptance → implémentation → refactor) en un seul run !")

...
─────────────────────────────────────────────────────────────────────────────
VARIANTE « TDD » (Test-Driven Development piloté par le plan : cycles red → green).

Différence avec la variante « verdict universel » Safe-Coding.py (dont ce script dérive) :
...
def write_fail_report(title: str, reason: str, blackboard: dict = None, details: str = ""):
    ...
    mm_audit.end("failed")
    try:
        lines = ["# Rapport d'échec — Safe-TDD", "", f"## {title}", "", "### Cause", reason.strip(), ""]
...
def main():
    ...
    if os.path.exists(SPEC_APPROVED_SENTINEL):
        os.remove(SPEC_APPROVED_SENTINEL)
    print("\n🏁 [CONGRATULATIONS] L'usine TDD a refermé tous ses cycles (red → green → refactor) en un seul run !")

# MAIsterMind (Version Windows via WSL)
...
1. **Installe** — une commande, zéro `chmod`, aucune installation de Python :
   `sh install.sh` depuis un terminal WSL 2 (Ubuntu) — prérequis apt, droits posés. Harness manquant ? `INSTALL.md`.
2. **Ouvre l'app** : double-clique sur `MAIsterMind.bat` — le navigateur s'ouvre seul.

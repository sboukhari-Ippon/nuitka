...
─────────────────────────────────────────────────────────────────────────────
CODING — pipeline de référence : verdict universel + surcouche (revue d'impact, vérificateur LLM, triage des cassures).

Base « verdict universel » (ex-Coding, retirée : ce script est désormais la référence), enrichie de
la surcouche Yolo — la logique d'arbitrage de Guided-Fix.py déplacée DANS le run :
  ...
  - Chemin vert (TOUTES les phases) : après le verdict mécanique, un Vérificateur LLM
    indépendant (contexte neuf) confronte le code réellement produit à la checklist de la
    phase du blackboard — la suite verte prouve « rien n'est cassé », pas « la phase a tout
    livré ». Il ne tamponne jamais DONE : un rejet consomme une tentative du codeur.
  - Chemin rouge (TOUTES les phases) : un Agent de Triage confronte les tests en échec à
...
Rappel de la base « VERDICT UNIVERSEL » — différence avec Coding-Without-Tests.py (qui reste en Palier 1) :
...
# PAS de bascule test/code dans ce script (contrairement à Coding-Without-Tests.py). La commande de vérification est TOUJOURS lue dans le champ
...
def write_fail_report(title: str, reason: str, blackboard: dict = None, details: str = "",
    ...
    mm_audit.end("failed")
    try:
        lines = ["# Rapport d'échec — Yolo", "", f"## {title}", "", "### Cause", reason.strip(), ""]
...
def run_production_phases(blackboard: dict, user_need: str, need_is_spec: bool = False):
    ...
    for phase in blackboard["phases"]:
        ...
        while not success and attempts < MAX_ATTEMPTS:
            ...
            # suite verte, la phase enchaîne ci-dessous (vérificateur LLM compris) ; sinon
            # la sortie rouge la plus fraîche redevient le feedback du codeur. Un timeout
            # d'infra garde sa branche dédiée (jamais traité comme un rouge).
            ...
            if is_ok:
                ...
                # prouve « rien n'est cassé », pas « la phase a tout livré ». Un agent
...
def main():
    ...
    if os.path.exists(IMPACT_APPROVED_SENTINEL):
        os.remove(IMPACT_APPROVED_SENTINEL)
    print("\n🏁 [CONGRATULATIONS] L'usine Coding a tout validé (verdicts mécaniques + vérificateur LLM) en un seul run !")

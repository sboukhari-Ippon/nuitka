...
─────────────────────────────────────────────────────────────────────────────
VARIANTE « YOLO » (verdict universel + surcouche Yolo : revue d'impact, vérificateur LLM, triage des cassures).

Dérive de Safe-Coding.py (copie intégrale, la base n'est PAS modifiée), enrichie de
...
Rappel de la base « VERDICT UNIVERSEL » (inchangée) — différence avec les variantes Agnostic Safe-Coding.py et Coding-Without-Tests.py (qui restent en Palier 1) :
...
RUNNER = resolve_runner(os.getcwd(), role="yolo")
...
def main():
    ...
    mm_audit.start(os.getcwd(), "yolo", RUNNER.name,
    ...
    if os.path.exists(IMPACT_APPROVED_SENTINEL):
        os.remove(IMPACT_APPROVED_SENTINEL)
    print("\n🏁 [CONGRATULATIONS] L'usine Yolo a tout validé (verdicts mécaniques + vérificateur LLM) en un seul run !")

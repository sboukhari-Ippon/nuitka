...
─────────────────────────────────────────────────────────────────────────────
TDD — pipeline de référence : cycles red → green → refactor + surcouche (revue d'impact, vérificateur LLM, triage des cassures).

Base TDD (ex-TDD, retirée : ce script est désormais la référence), enrichie de
...
Rappel de la base TDD — différence avec le pipeline « verdict universel » Coding.py :
...
def build_coder_prompt(phase: dict, blackboard: dict, user_need: str,
    ...
    if nature == RED_NATURE:
        ...
        prod_edit_policy = ("En phase green, tu ne crées et ne modifies AUCUN fichier de test "
                            ...
                            "révéler un bug d'un cycle antérieur à corriger).") + planned_test_changes_policy(phase)
...
def write_fail_report(title: str, reason: str, blackboard: dict = None, details: str = "",
    ...
    mm_audit.end("failed")
    try:
        lines = ["# Rapport d'échec — Test-First", "", f"## {title}", "", "### Cause", reason.strip(), ""]
...
def main():
    ...
    if os.path.exists(IMPACT_APPROVED_SENTINEL):
        os.remove(IMPACT_APPROVED_SENTINEL)
    print("\n🏁 [CONGRATULATIONS] L'usine Test-First a refermé tous ses cycles (red → green → refactor) en un seul run !")
